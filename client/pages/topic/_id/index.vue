          ...
          <div class="topic topic-article">
            <div class="topic-article__header">
              <span class="topic-article__title">
                <span>{{item.top ? '置顶' : item.good ? '精品' : item.tab && tabs[item.tab].name}}</span>
                {{item.title}}
              </span>
              <div class="topic-article__changes">
                <div>
                  <span>发布于 {{ item.create_at | timeAgo }}</span>
                  <span v-show="item.author">作者 {{item.author.loginname}}</span>
                  <span>{{item.visit_count}} 次预览</span>
                  <span>最后一次回复是 {{item.last_reply_at | timeAgo}}</span>
                  <span>来自 {{tabs[item.tab] && tabs[item.tab].name}}</span>
                </div>
                <div style="display: flex;">
                  <div class="topic-article__lazy-wrapper">
                    <lazy-wrapper :loading="loading"></lazy-wrapper>
                  </div>
                  <button
                    class="topic-article__collection"
                    :class="item.is_collect ? 'button--white' : 'button--green'"
                    @click="collectTopic"
                  >{{ item.is_collect ? '取消收藏': '收藏' }}</button>
                </div>
              </div>
              <div class="topic-article__manage" v-show="currentUser.loginname === user.loginname">
                <a :href="`/topic/${item.id}/edit`">
                  <i class="cn-icon-edit"></i>
                </a>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    user() {
      ...
      return (
        this.$store.state.users[loginname] || {
          loginname,
          avatar_url
        }
      );
  ...
  methods: {
    collectTopic() {
      ...
      this.$store.dispatch('COLLECT_TOPIC', {
        id: this.id,
        cancel: this.item.is_collect
      });
    },
